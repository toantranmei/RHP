...
<html lang="en">
<head>
	<title>Edit News Post | Social Network RHP Team</title>
	<meta charset="utf-8">
				...
				<div class="container">
					<div class="card">
						<div class="card-block">
							...
							<form action="<?php echo base_url(); ?>/Blog/saveEditNewsPost" method="POST" enctype="multipart/form-data">
								<?php foreach ($dataEditNewsPost as $key => $value): ?>
								
								<div class="form-group">
								    <label for="formGroupExampleInput">Title News Post</label>
								    <input type="hidden" name="id" value="<?php echo $value['id']; ?>">
								    <input type="text" name="name_news" class="form-control" id="formGroupExampleInput" value="<?php echo $value['name_news'] ?>" placeholder="Title News Post">
								</div>
								<div class="form-group">
								    <label for="formGroupExampleInput">Description News Post</label>
								    <input type="text" name="desc_news" class="form-control" id="formGroupExampleInput" value="<?php echo $value['desc_news'] ?>" placeholder="Description News Post">
								</div>
								<div class="form-group">
									<label for="formGroupExampleInput">Post Category</label>
										...
										<select name="id_cate" id="" class="form-control">
											...
											<?php if($value['id_cate'] == $cate['id']){ ?>
												<option value="<?php echo $value['id_cate']; ?>" selected><?php echo $dataNameCategory; ?></option>
											<?php } else{ ?>
							  	...
							  	<div class="form-group">
								    ...
								    <textarea name="content_news" value="<?php echo $value['content_news'] ?>" id="content_news" cols="30" rows="10">
								    	
								    </textarea>
								</div>
								<div class="form-group">
								    <label for="formGroupExampleInput">Thumbbaild Post</label>
								    <img src="<?php echo $value['image_news'] ?>" class="img-fluid" alt="">
								    <input type="hidden" name="old_image_news" value="<?php echo $value['image_news'] ?>">
								    <input type="file" name="image_news" class="form-control" id="formGroupExampleInput" placeholder="Title News Post">
								</div>
								<div class="form-group">
								    <input type="submit" class="btn btn-warning" id="formGroupExampleInput" value="Save News">
								</div>
								<?php endforeach ?>
							</form>
						</div>
